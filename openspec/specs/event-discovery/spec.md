...
Phase 5 member discovery: saved-events persistence and member feed / saved-upcoming query contracts in `@unveiled/db`, the authenticated SSR `/:locale/events` feed UI, SSR save/unsave mutations, `/:locale/saved`, and the USER navbar Saved affordance.
