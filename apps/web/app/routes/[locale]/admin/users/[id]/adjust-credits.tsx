import { adjustMemberCredits, getMemberDetail, isAdminMemberError } from "@unveiled/db";
import type { Context } from "hono";
import { createRoute } from "honox/factory";
import { AdminAdjustCreditsForm } from "../../../../../components/admin/AdminAdjustCreditsForm";
import {
...
function renderPage(
  ...
  return renderAdminPage(
    ...
    <AdminAdjustCreditsForm
      action={adminUserAdjustCreditsPath(options.locale, options.userId)}
...
export const POST = createRoute(async (c) => {
  ...
  try {
    ...
    return c.redirect(`${adminUserDetailPath(guard.locale, userId)}?ok=adjust-credits`, 302);
  } catch (error) {
    return renderPage(c, {
      locale: guard.locale,
      userId,
