import { saveEvent } from "@unveiled/db";
...
export const POST = createRoute(async (c) => {
  ...
  if (!eventId) {
    return c.redirect(`/${guard.locale}/events`, 302);
  }

  const { db } = getAuthOptions();
  try {
    await saveEvent(db, guard.session.user.id, eventId);
  } catch {
