...
**Processing location (current):** the admin **file picker** generates the five variants **in the browser with Pica** (`@unveiled/images/client`). The server **validates and stores** prebuilt WebP variants only — it does not resize. Admin event/partner UI is file-upload only (no paste-URL field on those forms). **JavaScript is required** for admin image supply. Source acceptance is **decode-success** (any image the browser can decode, including SVG); raw SVG/source bytes are never uploaded.
