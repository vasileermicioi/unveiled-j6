...
Events keep a **required singular primary image** (`events.image_id`) for cards, hero, and OG/JSON-LD. Optionally, admins may attach additional gallery photos via `event_gallery_images` (composite PK `(event_id, image_id)`, ordered by `sort_order`) with **no hard count cap**. Each gallery photo uses the **same five WebP variant pipeline** as the primary image (client Pica → server validate/store). Admin manage: `/admin/events/:id/gallery*` — thumbnail **grid**, drag-and-drop reorder with explicit **Save order** POST (the same POST also saves per-photo credits), checkbox select → SSR remove confirm (entry from Events list and/or event edit for any catalog event; the Featured events list is not a gallery entry point; see `sitemap/sitemap.md`, `features/admin-events.feature`). Public detail shows the gallery at page end when non-empty (`features/event-discovery.feature`, `ui/ui-component-map.md`). Gallery membership MUST NOT replace the primary hero. Schema: `database/schema-overview.md` → `event_gallery_images`.

## 8b. Optional image credit (`images.credit`)

Admins MAY record a free-text credit (trimmed, max 200; empty → `NULL`) on partner logos, event primary images, and gallery photos. The column is **`credit`**, never `source` (pipeline `source` / `source_url` stay upload vs remote). Admin fields: HeroUI `TextField` **Bildnachweis** / **Image credit** with hint “z. B. Foto: Name” / “e.g. Photo: Name”; names `image_credit` (partner logo + event primary), `image_credit_{index}` (gallery add, aligned with processed-file order), `image_credit_<imageId>` (gallery manage). Create persists credit with the new image. Keep existing file / staged-id retry: `updateImageCredit` on that id. Replace: credit applies to the **new** row only — omitted credit stores `NULL` (do not copy the old row).

Public `/events/:id` shows the stored string **as-is** (no auto `Foto:` / `Photo:` prefix) as a dark footer note on the primary hero when non-empty, as native `alt` (appended in parentheses after the base alt) and `title` on partner logo, event hero, and gallery thumbnails when non-empty, and as a footer note on that photo in the gallery lightbox when non-empty. Compact EventCards, map popups, admin table thumbs, and OG/JSON-LD MUST NOT show a visible credit caption. Empty/`NULL` credit MUST NOT render a caption placeholder. Admin event/partner variant previews use `title` on thumbs and a footer note on the lightbox. Gallery **manage** thumbs use `alt`/`title` plus a compact single-line credit input (no overlay, no hint under the tile); Save order still POSTs `image_credit_<imageId>`.
