...
Public `/events/:id` shows the stored string **as-is** (no auto `Foto:` / `Photo:` prefix) as a muted caption under the primary hero when non-empty, in the gallery lightbox for that photo when non-empty, and optionally under the DETAILS partner logo when logo credit is non-empty. Compact EventCards, map popups, admin table thumbs, and OG/JSON-LD MUST NOT show credit. Empty/`NULL` credit MUST NOT render a caption placeholder.
