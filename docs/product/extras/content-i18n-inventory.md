...
- Admin catalog publish chrome (`apps/web/app/lib/admin-content.ts`): `statusPublished` Veröffentlicht / Published; `statusDraft` Entwurf / Draft; `publishAction` / `unpublishAction` / `publishConfirm` / `unpublishConfirm`; `eventsPublishedFilter` Status / Status; `eventsPublishedAll` Alle / All; event confirm titles and bodies (`publishEventTitle`, `unpublishEventTitle` + matching `*Body`); `okPublish` Veröffentlichung gespeichert. / Publish status saved.; `okUnpublish` Veröffentlichung aufgehoben. / Unpublished. Featured membership is not separately draftable (no featured confirm copy in use).
- Admin event preview chrome (`apps/web/app/lib/admin-content.ts`): `previewAction` Vorschau / Preview; `previewBanner` Vorschau / Preview; `previewSurfaceDetail` Detail / Detail; `previewSurfaceBrowse` Events entdecken / Browse events; `previewSurfaceDiscover` Entdecken / Discover; `previewAudienceGuest` Gast / Guest; `previewAudienceMember` Mitglied / Member; `previewBrowseNote` Filter und Karte sind nicht Teil dieser Vorschau. / Filters and map are not part of this preview.; `previewOnlyCta` Nur Vorschau / Preview only; `previewDocumentTitle` `Vorschau: {title}` / `Preview: {title}`. Discover preview reuses Discover `livePreview` eyebrow/headline (see Discover/marketing bullet).
