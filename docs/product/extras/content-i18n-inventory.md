...
- Admin event multi-value fields (`apps/web/app/lib/admin-content.ts`): `languagesLabel` Sprachen/Languages; `languagesSearchPlaceholder` Sprachen suchen/Search languages; `targetAgeGroupsLabel` Altersgruppen/Age groups; series `builderWeekdaysLabel` Wochentage/Weekdays (+ `weekdayLabels`). Partner attribution on public detail uses denormalized `event.partnerName` + logo image alt (no separate copy keys).
