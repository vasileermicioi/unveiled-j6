...
| Optional **event gallery** (max 12 photos via `event_gallery_images`) uses the same image pipeline as the primary; primary `events.image_id` stays singular for cards/OG. Admin manage + public detail slider shipped; OG/JSON-LD still use the primary hero only. (**Variant format:** five WebP — see cutover below; historical six-JPEG wording superseded.) | `extras/image-uploads.md` §8a, `database/schema-overview.md`, `features/admin-events.feature`, `features/event-discovery.feature` |
