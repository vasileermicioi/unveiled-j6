...
| **Onboarding preference options:** location collects `country` / `city` / `zip_code` under prefilled Germany/Berlin (not Bezirk multi-select). Travel distance (`max_distance`) is **not** collected in onboarding or Vibes; any existing JSONB value is left untouched on location/preference saves and may still appear as legacy intel in Membership HQ when non-null. Not used to filter or rank `/events`. Accessibility is “Accessibility needed?” / “Barrierefreiheit benötigt?” with a Yes/Ja checkbox. Preferred languages use a searchable multi-select (DE/EN first; no Non-Verbal). Interests may include Other + free text (`interests_other`). | `features/onboarding.feature`, `features/profile.feature`, `database/schema-overview.md`, `extras/content-i18n-inventory.md` |
