...
| **Admin gallery manage entry (current):** Admins open gallery manage for any existing catalog event from the Events list and/or event edit page (`/admin/events/:id/gallery*`). Featured Discover membership is **not** required; the Featured list MAY keep a convenience gallery shortcut. Create-event has no gallery manage entry. | `features/admin-events.feature`, `ui/ui-component-map.md`, `extras/image-uploads.md` §8a, `apps/web/DEPLOYMENT.md` |
...
| **Event subtitles metadata (current):** `events.has_subtitles` boolean NOT NULL default false + nullable `subtitle_language` (ISO 639-1 alpha-2, broader than spoken-event allowlist). Admin native Subtitles checkbox + required native language `<select>` (full ISO list) when checked; independent of spoken languages / language-independent. Domain coerces language to null when off. Public detail shows a Subtitles DETAILS row only when on. No feed filter; no caption file upload. | `features/admin-events.feature`, `features/event-discovery.feature`, `database/schema-overview.md`, `@unveiled/db` `resolveEventSubtitles` |
...
| **Event multi-datetime + event-scoped booking (current):** Catalog events store a non-empty `date_times` list; denormalized `date_time` is next upcoming (or earliest if all past). **Admin add/remove datetime UI is parked** (`ALLOW_MULTI_DATETIME_UI = false` in `admin-event-form.ts`) — forms expose a single date/time row prefilled from primary `date_time`; schema/parsers remain multi-ready. Feed/map upcoming membership uses any future occurrence; cards, map popups, admin list columns, ICS, and confirmation email use next upcoming. Booking-eligible event detail can list all datetimes when present. **MVP booking remains event-scoped** (no datetime slot picker). | `features/admin-events.feature`, `features/event-discovery.feature`, `features/booking.feature`, `database/schema-overview.md`, `admin-event-form.ts` |
