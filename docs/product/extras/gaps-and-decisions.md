...
| **Onboarding preference options:** location uses the 12 official Berlin Bezirke (no informal shorthand); travel radius (`max_distance`) is not collected (legacy JSON key cleared on save). Accessibility is “Accessibility needed?” / “Barrierefreiheit benötigt?” with a Yes/Ja checkbox. Preferred languages use a searchable multi-select (DE/EN first; no Non-Verbal). Interests may include Other + free text (`interests_other`). | `features/onboarding.feature`, `features/profile.feature`, `database/schema-overview.md`, `extras/content-i18n-inventory.md` |
