...
| **Onboarding preference options:** location collects `country` / `city` / `zip_code` under prefilled Germany/Berlin (not Bezirk multi-select) plus required travel distance (`max_distance`) via native `input type="number"` (inclusive **1–50** km, `MAX_DISTANCE_MIN` / `MAX_DISTANCE_MAX`); labels EN “How far will you travel?” / DE “Wie weit bist du bereit zu fahren?” + `km`. Required on onboarding step 3 and when Vibes saves location fields. Capture-for-later / admin intel — not used to filter or rank `/events`. Accessibility is “Accessibility needed?” / “Barrierefreiheit benötigt?” with a Yes/Ja checkbox. Preferred languages use a searchable multi-select (DE/EN first; no Non-Verbal). Interests may include Other + free text (`interests_other`). | `features/onboarding.feature`, `features/profile.feature`, `database/schema-overview.md`, `extras/content-i18n-inventory.md` |
