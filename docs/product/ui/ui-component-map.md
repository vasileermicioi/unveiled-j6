...
| **Admin Featured events** | `/admin/featured*` | Tab label **Featured events** / **Empfohlene Events**. SSR list (`AdminFeaturedTable`) / search-add (`AdminFeaturedAddResults`) / remove-from-featured (keeps catalog event); list + add-results rows show primary-image thumbs (`small-320.webp` via `buildEventImageUrls`) or placeholder — missing/broken thumbs must not block actions; see `admin-events.feature` |
