...
| **Admin Featured events** | `/admin/featured*` | Tab label **Featured events** / **Empfohlene Events**. SSR list (`AdminFeaturedTable`) / search-add (`AdminFeaturedAddResults`) with Events-list-style filters (title/partner/language; language matches spoken or subtitle), column sort, Languages + Subtitles columns, Reset filters; remove-from-featured (keeps catalog event); list + add-results rows show primary-image thumbs (`small-320.webp` via `buildEventImageUrls`) or placeholder — missing/broken thumbs must not block actions; see `admin-events.feature` |
