...
- **Default page/section header:** shared `PageSectionHeader` (on-yellow eyebrow + headline + rule) for Discover, FAQ, auth (`AuthPageLayout`), member browse, book/confirm, and waitlist surfaces. **Optional bordered card hero:** `PageHero` for long-form marketing/legal intros (e.g. How it works). Membership `/membership` keeps its bordered marketing hero card; plan benefits below it are a **vertical icon-bullet list** (distinct icon per perk), not a horizontal three-up card strip. Do not invent one-off FAQ/auth/book heroes.
