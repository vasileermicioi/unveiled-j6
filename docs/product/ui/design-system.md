...
- **Multi-value allowlists** (onboarding preferences, admin event age groups) → native **checkbox multi-select** (`CheckboxMultiSelect` island; optional client-side search filter). Native HTML `<select multiple>` is **not** the preferred pattern for new multi-value admin fields.
