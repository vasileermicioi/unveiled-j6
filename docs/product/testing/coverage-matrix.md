...
| `admin-users.feature` | List all members | `e2e/specs/admin-users.spec.ts` · `Scenario: List all members` | `pass` | Needs `DATABASE_URL` + `E2E_ADMIN_*`; merged Member column + Created date, default member-asc |
| `admin-users.feature` | Search members | `e2e/specs/admin-users.spec.ts` · `Scenario: Search members` | `pass` | Needs `DATABASE_URL` + `E2E_ADMIN_*`; filter bar (`q`, role, subscription) + URL query params (numeric ranges, created range) |
| `admin-users.feature` | View a member's collapsed summary | `e2e/specs/admin-users.spec.ts` · `Scenario: View a member's collapsed summary` | `pass` | Merged Member cell + Created date; list row columns (role, subscription, credits, …) |
| `admin-users.feature` | Member rows show combined name and email | `e2e/specs/admin-users.spec.ts` · `Scenario: Member rows show combined name and email` | `pass` | Needs `DATABASE_URL` + `E2E_ADMIN_*`; merged name-link + email line |
| `admin-users.feature` | Created column shows registration date | `e2e/specs/admin-users.spec.ts` · `Scenario: Created column shows registration date` | `pass` | Needs `DATABASE_URL` + `E2E_ADMIN_*`; Europe/Berlin calendar day |
| `admin-users.feature` | Sort members via header links | `e2e/specs/admin-users.spec.ts` · `Scenario: Sort members via header links` | `pass` | Needs `DATABASE_URL` + `E2E_ADMIN_*`; toggle + new-column defaults, filters preserved |
| `admin-users.feature` | Filter members by subscription | `e2e/specs/admin-users.spec.ts` · `Scenario: Filter members by subscription` | `pass` | Needs `DATABASE_URL` + `E2E_ADMIN_*`; enum dropdown incl `NONE` |
| `admin-users.feature` | Filter members by numeric range | `e2e/specs/admin-users.spec.ts` · `Scenario: Filter members by numeric range` | `pass` | Needs `DATABASE_URL` + `E2E_ADMIN_*`; credits/bookings/event-opens min/max via URL query params |
| `admin-users.feature` | Filter members by created date range | `e2e/specs/admin-users.spec.ts` · `Scenario: Filter members by created date range` | `pass` | Needs `DATABASE_URL` + `E2E_ADMIN_*`; `createdFrom`/`createdTo` Berlin days via URL query params |
| `admin-users.feature` | Sort and filter compose through pagination | `e2e/specs/admin-users.spec.ts` · `Scenario: Sort and filter compose through pagination` | `pass` | Needs `DATABASE_URL` + `E2E_ADMIN_*`; filter+sort persist across page navigation/clamp |
| `admin-users.feature` | Reset filters | `e2e/specs/admin-users.spec.ts` · `Scenario: Reset filters` | `pass` | Needs `DATABASE_URL` + `E2E_ADMIN_*`; reset link returns to default list |
...
| `event-discovery.feature` | Inactive member nav shows membership CTA | `e2e/specs/event-discovery.spec.ts` · `Scenario: Inactive member nav shows membership CTA` | `pass` | Fresh signup; no Browse events / credits; Start membership → `/membership` |
| `event-discovery.feature` | Browse events filters are collapsed by default | `e2e/specs/event-discovery.spec.ts` · `Scenario: Browse events filters are collapsed by default` | `pass` | Filter fields hidden until expand |
...
| `static-pages.feature` | Guest marketing home is the locale home page | `e2e/specs/static-pages.spec.ts` · `Scenario: Guest marketing home is the locale home page` | `pass` | v3 hero + 29 € offer card + signup CTA; slim header / Discover+FAQ footer |
...
| `static-pages.feature` | V3 landing rail shows login-gated teasers | `e2e/specs/static-pages.spec.ts` · `Scenario: V3 landing rail shows login-gated teasers` | `pass` | Live rail ≤3 teasers, no credit figures or `/events/:id` links in rail scope, live + locked CTAs → login; proximity selectors only |
