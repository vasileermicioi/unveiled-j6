...
| `admin-events.feature` | Create walks three steps | `e2e/specs/admin-events.spec.ts` · `Scenario: Create walks three steps` | `pass` | R2 env-skip (partner logo); Next through General → Date & tickets → Image |
| `admin-events.feature` | Create submit is on the image step | `e2e/specs/admin-events.spec.ts` · `Scenario: Create submit is on the image step` | `pass` | R2 env-skip; Anlegen/Create only on step 3; POST includes earlier steps |
| `admin-events.feature` | Edit can jump to image | `e2e/specs/admin-events.spec.ts` · `Scenario: Edit can jump to image` | `pass` | R2 env-skip (create source); numbered step jump; no POST until Save |
...
| `admin-events.feature` | Check Subtitles reveals language select | `e2e/specs/admin-events.spec.ts` · `Scenario: Check Subtitles reveals language select` | `pass` | R2 env-skip (partner logo); native checkbox + native select |
...
| `admin-events.feature` | List featured events | `e2e/specs/admin-events.spec.ts` · `Scenario: List featured events` | `pass` | Needs `E2E_ADMIN_*`; asserts **Featured events** / **Featured partners** tab labels; thumb proximity covered when rows exist via remove scenario |
...
| `admin-events.feature` | Admin remove from featured keeps catalog event | `e2e/specs/admin-events.spec.ts` · `Scenario: Admin remove from featured keeps catalog event` | `pass` | Needs `E2E_ADMIN_*` + R2; asserts list-row thumb (`small-320.webp`) proximity before remove |
...
| `event-discovery.feature` | Guest sees partner opening hours | `e2e/specs/event-discovery.spec.ts` · `Scenario: Guest sees partner opening hours` | `pass` | Needs `DATABASE_URL`; temporarily enables hours on seeded partner and restores after |
